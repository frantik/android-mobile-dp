...
public class AboutActivity extends SherlockPreferenceActivity {
    ...
    public void onBuildHeaders(List<Header> target) {
        try {
            if (getResources().getIdentifier("about_header", "xml", getPackageName()) <= 0)
                return;

            loadHeadersFromResource(R.xml.about_header, target);
        } catch (Exception e) {
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case android.R.id.home:
            EasyTracker.getTracker().sendEvent("About Tab", "Click", "Home", 0L);
            finish();
            return true;
        default:
