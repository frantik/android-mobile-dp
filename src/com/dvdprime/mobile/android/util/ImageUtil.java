...
public class ImageUtil {
    ...
    /**
     * Bitmap을 최대 길이로 비교하여 최대 길이가 크면 해당 인자값의 길이로 맞춰서 비례적인 크기 조절을 한다. (말이 쫌 어렵네... )
     * 
     * @param bitmap
     * @param maxDimension
     * @return
    ...
    public static Bitmap resizeBitmap(Bitmap bitmap, final int maxDimension) {
        ...
        if (bitmapWidth > maxDimension || bitmapHeight > maxDimension) {
            final float scale = Math.min((float) maxDimension / (float) bitmapWidth, (float) maxDimension / (float) bitmapHeight);
            final int scaledWidth = (int) (bitmapWidth * scale);
            final int scaledHeight = (int) (bitmapHeight * scale);

            Log.d(TAG, "Resizing from: " + String.valueOf(bitmapWidth) + "x" + String.valueOf(bitmapHeight) + " to " + String.valueOf(scaledWidth) + "x" + String.valueOf(scaledHeight));
     ...
     * @return
     */
    public static Bitmap ResizeCenterInBitmap(Bitmap bitmap, int newWidth, int newHeight) {
        ...
        Bitmap cropedBitmap = Bitmap.createBitmap(bitmap, sourceX, sourceY, newWidth, newHeight);
    ...
    // {
    // input =
    // ContextHolder.getInstance().getContext().getContentResolver().openInputStream(uri);
    ...
    // // Compute the scale.
    // int scale = 1;
    // while ((options.outWidth / scale > size) || (options.outHeight / scale >
    // size))
    // {
    // scale *= 2;
    // }
    ...
    // catch (IOException e)
    // {
    // if (MessageHelper.DEBUG) Log.w(TAG, e);
    //
    // return null;
    // }
    // finally
    ...
    // {
    // File tempFile = File.createTempFile("tmpbmp", ".jpg",
    // ContextHolder.getInstance().getContext().getFilesDir());
    // byte[] buffer = new byte[16 * 1024];
    // BufferedOutputStream out = new BufferedOutputStream(new
    // FileOutputStream(tempFile), buffer.length);
    ...
    // else
    // {
    // Log.d(TAG, "Did not find picture in MediaStore : " +
    // imageFile.toString());
    // }
    //
    // return uri;
    ...
    public static int computeSampleSize(BitmapFactory.Options options, int target) {
        ...
        Log.d(TAG, "for w/h " + w + "/" + h + " returning " + candidate + "(" + (w / candidate) + " / " + (h / candidate));
    ...
    public static Bitmap rotate(Bitmap bitmap, int degrees) {
        try {
            ...
            Toast.makeText(DpApp.getContext(), "메모리부족", 0).show();
        }
        return bitmap;
    }
}
